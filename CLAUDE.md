...
- **question (ฟ้า)** = UI ควิซปกติ (`isQuizKind`) → `answerQuiz(...)` ได้เหรียญปกติ (ไม่ใช่เหรียญกษัตริย์) · รองรับ 50:50/ข้ามคำถาม/×2 · ตอบผิดเสียหัวใจ
  - **ฟีดแบ็กตอบถูก/ผิด 3 ชั้น:** เสียง (`sfx.correct/wrong`) + **ตราประทับ ✓/✕ เด้งกลางจอ** (`stamp` state, `position:fixed` z=130, `pointerEvents:none`, จางเองใน 1.1 วิ) + แบนเนอร์เขียว/แดง + คอนเฟตติ + `quizPop`/`quizShake` ที่ปุ่มคำตอบ
