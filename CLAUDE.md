...
- **UI อยู่ในแถบขวาเท่านั้น ห้ามลอยทับกระดาน** — `RoomClock` (รหัสห้อง+เวลาถอยหลัง) เสียบในแถวไอคอน ⚙️🏠 ที่ฝั่งซ้ายว่างอยู่แล้ว · `RoomStandings` เป็นการ์ดใต้แถวนั้น (โชว์ 4 ทีมแรก + **ทีมเราเสมอ** แม้อันดับหลุดจาก 4 แรก) · เดิมลอยทับมุมซ้ายล่างแล้วบังช่องบนกระดานจริง
- **เน็ตหลุด = เกมในเครื่องเดินต่อปกติ** แค่แผงอันดับขึ้น "ขาดการเชื่อมต่อ" (`room.offline`) แล้ว retry รอบหน้า — ห้ามให้ห้องมาบล็อกการเล่น
...
- **📱 โหมด QR ตอบบนมือถือ (privacy):** `settings.qrAnswerMode` (Teacher Mode) — ช่อง**ฟ้า/สาระ** พอจั่วใบเสร็จ tablet โชว์ **QR แทน UI ควิซ** (คำถามไม่โผล่บนจอกลาง = ผู้เล่นอื่นไม่เห็น) ผู้เล่นสแกนด้วยมือถือตัวเอง → เปิด `answer.html` (แยก entry โหลดเบา ~48KB ไม่ลาก MindAR) ตอบเห็นผลทันที
  ...
  - **🔒 วงจรชีวิตของ challenge — ต้องมี "ปิด" ไม่ใช่แค่ "เปิด" (แก้บั๊กสแกนคำถามเก่า):**
    - `registerChallenge` = **เปิดข้อใหม่ → ล้าง `answered/correct/used_items` ทิ้งเสมอ** (จำเป็นกับ StrictMode ดูด้านล่าง)
    - `closeChallenge(id)` (`POST {id, close:true}`) = แท็บเล็ตเลิกฟังผล → `answered = 1` โดยไม่แตะ `correct` · เรียกจาก **2 จุด**: ปุ่มผลที่ครูกดเอง (`manual`) และ cleanup ของ effect poll (ครอบคลุมจบเทิร์น/ปิดการ์ด/ออกจากเกม)
    - `GET ?payload=1` **ตอบ 410 เมื่อ `answered = 1`** → มือถือสแกน QR เก่าแล้วขึ้น "ข้อนี้จบไปแล้ว" แทนที่จะได้โจทย์สด ๆ มาตอบใหม่ · แยกจาก 404 (QR ผิดจริง) ด้วย `ChallengeClosedError`
    - **กับดักที่เคยพลาด:** ปุ่ม "ตอบถูก/ตอบผิด" ที่ครูกดบนแท็บเล็ต เดิม**จบเกมฝั่งแท็บเล็ตอย่างเดียว ไม่เคยบอก server** → แถวค้าง `answered = 0` ตลอด → poll ฝั่งมือถือ (ด้านล่าง) ไม่มีวันทำงาน และ QR เก่ายังสแกนเปิดได้เรื่อย ๆ
    - **⚠️ `closeChallenge` ใน cleanup ต้องข้ามถ้าเพิ่งแสดงไม่ถึง 1.2 วิ** — `React.StrictMode` (เปิดทุก entry) mount ซ้ำทันที ทำให้ cleanup รอบแรกยิง close **แข่งกับ** `registerChallenge` ของรอบสอง ถ้า close มาถึงทีหลัง ข้อจะค้างปิดตั้งแต่ยังไม่มีใครสแกน = **เกมค้างรอคำตอบที่ไม่มีวันมา** · การ์ดจริงอยู่บนจอเป็นวินาทีเสมอ จึงใช้เวลาแสดงจริงแยกสองเคสนี้
